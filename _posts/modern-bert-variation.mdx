---
title: "Computer Chess: The ModernBERT Variation"
date: 2025-03-27
...
If you just want to play around with the tokenizer, [all of the code is open-source](https://github.com/DVDAGames/pgn-tokenizer), the Python library is published to PyPI as [`pgn-tokenizer`](https://pypi.org/project/pgn-tokenizer/), and a beta version of the TypeScript library is published to npm as [`@dvdagames/pgn-tokenizer`](https://www.npmjs.com/package/@dvdagames/pgn-tokenizer).
...
This dataset didn't fall into either category that I mentioned earlier, despite the underlying source using [some truly bizarre formatting](https://chess-research-project.readthedocs.io/en/latest/#description-of-the-simplified-format). Maybe it was less bizarre almost a decade ago when the research was originally published? I'm sure the labeling was in service of some research goal, but it means the dataset isn't as valuable for general purposes.
...
After scripting out downloading and some more cleaning to remove games without moves or games with unexpected formatting, I had nice, simple PGN strings to work with. The next day, the original dataset I was building on top of vanished from Kaggle. Luckily, I still had it cached locally and was able to publish my cleaned [PGN dataset](https://huggingface.co/datasets/InterwebAlchemy/pgn-dataset) to Hugging Face.
...
As I tested various models, I started to think that something prevents the models from attaching appropriate semantic meaning to individual moves PGN strings.

Looking at how the tokenizers for popular model architectures break a PGN string down into tokens really illustrates how that could be an issue:
...
Emboldened by success with the tokenizer, I decided it was time to push things a bit further and train a language model.
...
There are probably so many more, but these were the big ones that stand out in hindsight.

I hit a wall pretty quickly, and ended up setting this project aside for a bit so that I could fully immerse myself in my RC batch.

## Endgame

I've recently picked the model training back up, and I'm excited to say that there has been a lot of progress on training a language model that only understands PGN strings.
...
It would be simple to just have it spit out PGN-like tokens - and that's what my first iteration did, but I'm trying to see if there's a way to encode an understanding of the game by only using the tokens. This involves weighting the tokens based on their position in the PGN string, and reducing the weight of tokens like the turn number because they are more prevalent in the training data, but only valid in specific token positions.

<TrainingVisualizer />

It's also important to keep the context of the entire game in mind when making each move, so we aren't just training on a subset of PGN tokens at a time, but a progressively longer string of tokens as context for each move in each game of the training set.

In Chess, every previous move influences the future state of the game.

The basic training implementation seeks to:

1. Start wtih a baseline sample of short games as a warmup with a lower learning rate increasing gradually
2. Move to a set of medium length games with a normal learning rate
3. Move a set of common openings with a higher learning rate
4. Finish with a set of longer games with a normal learning rate
...
I'm still working through the training process, and working out the challenges of disparate environments. Training code that runs on my Macbook M4 Pro doesn't necessarily work when I'm loading it up on a cloud provider with a small GPU like a T4 - and the various PyTorch or CUDA error messages aren only debuggable in the cloud environment, which makes the debugging cycle and resolution time stretch out much longer than it should.

This is a work-in-progress, but I can't wait to share the model and the results of testing my hypothesis with you in the near future.
